...
/*   Updated: 2025/04/13 16:10:35 by syl              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
float *create_rotation_from_vector(t_coord *to)
{
    t_coord *axis;
    float angle;
    float c, s, t;
    ...
    if (fabs(angle) < EPSILON)
    {
		m_rot = create_indentity_matrix_44();
    ...
    m_rot = create_matrix(4, 4);

	m_rot = matrix_rotation_rodrigues(axis, angle)

    // Matrice 3x3 de rotation de Rodrigues + extension en 4x4
    matrix_fill(m_rot, 0, 0, t*x*x + c);
    matrix_fill(m_rot, 0, 1, t*x*y - s*z);
    matrix_fill(m_rot, 0, 2, t*x*z + s*y);
    matrix_fill(m_rot, 0, 3, 0);

    matrix_fill(m_rot, 1, 0, t*x*y + s*z);
    matrix_fill(m_rot, 1, 1, t*y*y + c);
    matrix_fill(m_rot, 1, 2, t*y*z - s*x);
    matrix_fill(m_rot, 1, 3, 0);

    matrix_fill(m_rot, 2, 0, t*x*z - s*y);
    matrix_fill(m_rot, 2, 1, t*y*z + s*x);
    matrix_fill(m_rot, 2, 2, t*z*z + c);
    matrix_fill(m_rot, 2, 3, 0);
