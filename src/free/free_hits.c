...
/*   Updated: 2025/05/15 15:49:35 by cmegret          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void free_pix_hits(t_hits ***hits, t_program_context *context)
{
	...
	// Libération des tableaux de hits
	free_hits_type(hits, 0, 2); // background (alloué pour 2 éléments)
	if (context->num_obj)
	{
		free_hits_type(hits, 1, context->num_obj->sphere + 1); // +1 pour le NULL terminal
